...
export class AudioManager {
  ...
  async startCapture(onData) {
    ...
    this.workletNode.port.onmessage = (event) => {
      ...
      const resampled = resampleLinear(event.data.data, this.nativeSampleRate, CAPTURE_SAMPLE_RATE);
  ...
  stopCapture() {
    this.onAudioData = null;
    if (this.workletNode) { this.workletNode.port.close(); this.workletNode.disconnect(); this.workletNode = null; }
  ...
  playAudio(pcmData) {
    ...
    const audioBuffer = this.playbackContext.createBuffer(1, float32Samples.length, PLAYBACK_SAMPLE_RATE);
  ...
  destroy() {
    this.stopCapture();
    this.stopPlayback();
    if (this.captureContext) { this.captureContext.close().catch(() => {}); this.captureContext = null; }
  ...
  schedulePlayback(buffer) {
    ...
    // Larger lookahead buffer (0.08s) to prevent gaps between chunks
    const startTime = Math.max(now + 0.08, this.scheduledPlaybackTime);
    const duration = buffer.duration;
    const fadeDuration = Math.min(0.004, duration / 6);

    gainNode.gain.setValueAtTime(0, startTime);
    gainNode.gain.linearRampToValueAtTime(1, startTime + fadeDuration);
    gainNode.gain.setValueAtTime(1, startTime + Math.max(fadeDuration, duration - fadeDuration));
    ...
    source.onended = () => { this.activePlaybackSources.delete(source); source.disconnect(); gainNode.disconnect(); };

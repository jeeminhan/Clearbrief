...
export class AudioManager {
  ...
  schedulePlayback(buffer) {
    ...
    // Schedule chunks back-to-back with no gap — no gain fades between chunks
    // to avoid micro-silences that cause choppy playback
    const startTime = Math.max(now, this.scheduledPlaybackTime);

    source.connect(this.playbackContext.destination);
    this.activePlaybackSources.add(source);

    source.onended = () => { this.activePlaybackSources.delete(source); source.disconnect(); };

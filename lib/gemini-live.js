...
export class GeminiLiveClient {
  ...
  connect() {
    ...
    this.ws.onclose = (event) => {
      ...
      if (wasActive && event.code !== 1000) {
        this.config.onError(new Error(`Connection lost (code ${event.code})`));
      }
    };
  }
  ...
  async handleMessage(raw) {
    ...
    // Activity detection events — user started/stopped speaking
    if (msg.realtimeInput?.activityHandling) {
      const activity = msg.realtimeInput.activityHandling;
